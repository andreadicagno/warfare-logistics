...
describe('TerrainGenerator', () => {
  ...
  describe('generate', () => {
    ...
    it('produces water hexes at low elevation', () => {
      ...
      const waterCells = [...cells.values()].filter(
    ...
    it('produces mountains at high elevation', () => {
      ...
      const mountains = [...cells.values()].filter(
        (c) => c.terrain === TerrainType.Mountain,
      );
    ...
    it('mountainous geography produces more mountains than plains geography', () => {
      ...
      const countMountains = (
        cells: Map<string, { terrain: TerrainType }>,
      ) =>
        [...cells.values()].filter(
          (c) =>
            c.terrain === TerrainType.Mountain ||
            c.terrain === TerrainType.Hills,
        ).length;
      expect(countMountains(mountainous)).toBeGreaterThan(
        countMountains(plains),
      );
    });
  });
  ...
  describe('assignTerrain', () => {
    it('returns Water for low elevation', () => {
      expect(TerrainGenerator.assignTerrain(0.1, 0.5, 'mixed')).toBe(
        TerrainType.Water,
      );
    });

    it('returns Mountain for high elevation', () => {
      expect(TerrainGenerator.assignTerrain(0.9, 0.5, 'mixed')).toBe(
        TerrainType.Mountain,
      );
    });

    it('returns Marsh for low-mid elevation and high moisture', () => {
      expect(TerrainGenerator.assignTerrain(0.28, 0.8, 'mixed')).toBe(
        TerrainType.Marsh,
      );
    });

    it('returns Forest for mid elevation and high moisture', () => {
      expect(TerrainGenerator.assignTerrain(0.45, 0.8, 'mixed')).toBe(
        TerrainType.Forest,
      );
    });

    it('returns Plains for mid-low elevation and low moisture', () => {
      expect(TerrainGenerator.assignTerrain(0.4, 0.2, 'mixed')).toBe(
        TerrainType.Plains,
      );
    });

    it('returns Hills for mid-high elevation and low moisture', () => {
      expect(TerrainGenerator.assignTerrain(0.65, 0.2, 'mixed')).toBe(
        TerrainType.Hills,
      );
    });
  });
